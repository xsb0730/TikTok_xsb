    ...
    private fun initRecyclerView(){
        ...
        adapter= GridVideoAdapter(
            ...
            onItemClick = {video,position->
                // 点击卡片跳转到全屏播放
                val video=viewModel.getCurrentVideoList()
                VideoPlayAdapter.start(requireContext(),videoList,position)
            },
            ...
            when (resource) {
                ...
                is Resource.Success -> {
                    ...
                    resource.data?.let { videos ->
                        ...
                        Toast.makeText(context, "加载了 ${videos.size} 条数据", Toast.LENGTH_SHORT).show()
                    }
                }
                is Resource.Error -> {
                    binding.refreshLayout.isRefreshing = false
