package com.example.tiltok_xsb.ui.fragment

import android.os.Bundle
import android.view.View
import com.example.tiltok_xsb.base.BaseBindingFragment
import com.example.tiltok_xsb.databinding.FragmentMallBinding
import com.example.tiltok_xsb.utils.SwipeGestureHelper


class MallFragment : BaseBindingFragment<FragmentMallBinding>({FragmentMallBinding.inflate(it)}), IScrollToTop {

    private var swipeGestureHelper: SwipeGestureHelper? = null

    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        super.onViewCreated(view, savedInstanceState)
        setupSwipeGesture()
    }

    private fun setupSwipeGesture() {
        swipeGestureHelper = SwipeGestureHelper(
            context = requireContext(),
            onSwipeLeft = {
                // 向左滑动，切换到推荐页（position = 5）
                (parentFragment as? MainFragment)?.switchTab(5)
            },
            onSwipeRight = {
                // 向右滑动，切换到关注页（position = 3）
                (parentFragment as? MainFragment)?.switchTab(3)
            }
        )
        swipeGestureHelper?.attachToRecyclerView(binding.recyclerView)
    }



    override fun scrollToTop() {}

    override fun onDestroyView() {
        super.onDestroyView()
        swipeGestureHelper = null
    }
}